...
- 통계표: `DT_1B81A01` 출생아수. 항목 `itmId=T1`(계). `T2`=남자 `T3`=여자
  - 표 이름은 **"시군구/성/월별 출생"** 입니다. `prdSe=Y` 로 연간 집계를 받을 수 있습니다
  - **`objL1=ALL` 이면 시군구 264개가 함께 옵니다.** 시도만 받으려면 2자리 코드 18개를 명시합니다
    `objL1=00+11+21+22+23+24+25+26+29+31+32+33+34+35+36+37+38+39`
  - `C1` 이 2자리면 시도(전국 `00` 포함), 5자리면 시군구입니다
  - **KOSIS 의 `C1` 은 행정구역코드가 아닙니다** (부산 `21` vs 행정구역코드 `26`). 지역 매핑은 `C1_NM` 으로 합니다
  - **`itmId=T20` 은 존재하지 않습니다.** 쓰면 `err=21` 이 납니다
...
- 주의: `UNIT_NM` 이 표의 8개 항목 단위를 합친 문자열로 옵니다. 단, `DT_1B81A01` 은 `명` 하나로 옵니다
- 최종 확인: 2026-09-16 실시간 호출 검증 완료 — `DT_1B81A21` 198행 / `DT_1B81A01` 198행(2015~2025, 결측 0건, 값 2,708~438,420)
